...
export const generateWonderReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: profile }, { data: memories }, { data: msgs }, { data: feedback }] =
      await Promise.all([
    ...
    const lovedBlock = smiles.length
      ? smiles
          ...
          .map(
            (m) =>
              `😊 [${m.role}] ${m.content.slice(0, 240)}${fbMap.get(m.id)?.note ? ` — note: "${fbMap.get(m.id)!.note}"` : ""}`,
          )
          .join("\n")
    ...
    const res = await fetch(GATEWAY, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Write this week's WONDER REPORT for ${profile?.display_name || "them"} — a short, beautiful summary of the most interesting ideas, threads, and questions. Format with markdown. 4-6 short sections with evocative headings.
